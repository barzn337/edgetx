...
echo "=== Step $((STEP++))/${STEPS}: Installing Python setuptools and wheel ==="
...
echo "=== Step $((STEP++))/${STEPS}: Fetching and installing Python package bcj-cffi ==="
...
echo "=== Step $((STEP++))/${STEPS}: Fetching and installing Python package pyppmd ==="
...
echo "=== Step $((STEP++))/${STEPS}: Installing Python package py7zr in version 0.16.1 ==="
...
echo "=== Step $((STEP++))/${STEPS}: Installing Python package aqtinstall in version 1.2.5 ==="
...
echo "=== Step $((STEP++))/${STEPS}: Installing Qt build environment ==="
python -m aqt install 5.12.9 windows desktop win64_mingw73
if [[ $PAUSEAFTEREACHLINE == "true" ]]; then
  echo "Step finished. Please check the output above and press Enter to continue or Ctrl+C to stop."
fi

echo "=== Step $((STEP++))/${STEPS}: Installing zip package ==="
